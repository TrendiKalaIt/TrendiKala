...
//       setLoading(true);
//       try {
//         const res = await axios.get('/api/addresses/my', {
...
//                 <div>
//                   {addr.streetAddress}
//                   {addr.apartment ? `, ${addr.apartment}` : ''}, {addr.townCity}
//                 </div>
//                 <div>
...
//   loadingSubmit, handlePlaceOrder,
//   selectedAddress,
// }) => (
//   <div>
...
//   if (isGuest) {
//     // Guest user: Always show AddressForm, no saved addresses, no add/save buttons
//     return (
//       <div className="mb-8">
//         <h2 className="text-2xl text-green-600 font-semibold mb-4">Delivery Address</h2>
//         <AddressForm
//           guestMode={true}           // Important: hide save button in AddressForm
//           address={guestAddress}
//           onAddressChange={setGuestAddress}  // AddressForm should call this on change
//         />
//       </div>
//     );
//   }

//   // Logged-in user UI with saved addresses + add form toggle
//   return (
//     <div className="mb-8">
//       <h2 className="text-2xl text-green-600 font-semibold mb-4">Delivery Address</h2>
//       {loading ? (
...
//   // For guest user, save their address here
...
//   // For guest, selectedAddress is guestAddress; for logged-in use selectedAddress state
...
//   setLoadingSubmit(true);
//   try {
//     // Agar guest user hai toh token na bhejo
//     const result = await dispatch(
//       isGuest
//         ? placeOrder({ orderPayload })            // guest: no token
//         : placeOrder({ orderPayload, token })     // logged-in user: send token
//     );
...
const CheckoutSection = ({
  ...
  <div>
      ...
      {[
        ...
        <div
          ...
          className={`flex items-center p-4 rounded-lg border cursor-pointer transition duration-200 ${
            paymentMethod === id ? 'border-green-500 bg-green-50' : 'border-gray-300'
          }`}
          onClick={() => setPaymentMethod(id)}
        >
          <input
    ...
    <button
      onClick={handlePlaceOrder}
      disabled={loadingSubmit || !selectedAddress}
      className={`mt-8 w-full bg-green-600 text-white py-2 rounded-lg font-semibold text-lg shadow-md transition duration-300 ease-in-out ${
        loadingSubmit || !selectedAddress ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-700'
      }`}
    >
      {loadingSubmit ? 'Placing Order...' : 'Place Order'}
    </button>
